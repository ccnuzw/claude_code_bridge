...
- **Release smoke 跟随新 ask 契约**：真实平台 smoke 用 `ccb pend --watch` 观察 job，在独立 nested stress ask 中使用 `--silence`，并兼容 macOS Bash。
...
- fastpath stress 避免空数组展开，保持 macOS Bash 下 user-origin ask 可可靠提交。
- 保留 active-agent 独立 ask 使用 `--silence`，同时继续覆盖 user-origin plain `ask`。
