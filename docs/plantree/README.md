...
| [agentic-loop-workflow](plans/agentic-loop-workflow/README.md) | In progress | Design and land a state-machine-driven multi-agent workflow loop where frontend/frontdesk stays thin, planner owns macro plan-tree state, orchestrator manages mount topology plus ask-first execution, and scripts own authority transitions. |
