...
- `ccb layout status --json` may include best-effort tmux observation
  metadata for each observed pane, including `pane_index`, `pane_width`, and
  `pane_height`. These fields are diagnostics evidence for layout validation
  and smoke tests, not workflow authority; sidebar/tool panes may appear in
  observed runtime pane counts and must be distinguished from configured agent
  panes by CCB identity metadata.
